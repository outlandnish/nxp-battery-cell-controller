...
/**
  ...
  * @retval None
  */
spi_init_e spi_init_custom(spi_t *obj, uint32_t speed, SPIMode mode, uint8_t msb)
{
  if (obj == NULL) {
    return SPI_OBJ_NULL;
  }
  ...
  if (obj->spi != SUBGHZSPI) {
    ...
    if (spi_mosi == NP || spi_miso == NP || spi_sclk == NP || spi_ssel == NP) {
      if (spi_miso == NP && obj->is_duplex) {
        core_debug("ERROR: at least one SPI pin has no peripheral\n");
        return SPI_MISSING_PERIPHERAL;
      }
    }
