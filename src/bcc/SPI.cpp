...
/**
  ...
  */
bool SPIClass::begin(void)
{
  _spi.handle.State = HAL_SPI_STATE_RESET;
  _spiSettings = SPISettings();
  auto error = spi_init_custom(&_spi, _spiSettings.clockFreq,
           _spiSettings.dataMode,
           _spiSettings.bitOrder);
  if (error != SPI_INIT_OK) {
    Serial.printf("SPI init error: %d\n", error);
  }

  init = error == SPI_INIT_OK;
  return init;
}
...
void SPIClass::setBitOrder(BitOrder bitOrder)
{
  ...
  spi_init_custom(&_spi, _spiSettings.clockFreq,
           _spiSettings.dataMode,
           _spiSettings.bitOrder);
}
